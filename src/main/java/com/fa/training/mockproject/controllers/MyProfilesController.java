...
public class MyProfilesController {
    ...
    public ModelAndView createProfile(@ModelAttribute("user") UserAccountsLoginDTO userAccountsLoginDTO, ModelAndView modelAndView) {
        if (userAccountsLoginDTO.getEmployees().getDomains().getDomainName().equals("New Account")) {
            modelAndView.addObject("message", "Please contact Manager to set your Domain!");
            modelAndView.setViewName("myprofiles/index");
        } else {
            modelAndView.addObject("periodPatternList", periodPatternService.findAllByOrderByYearAscNameAsc());
            modelAndView.addObject("competencyRankingPatternsList", competencyRankingPatternsService.findAllByJobRoles_Domains_DomainIdAndActiveStatus(userAccountsLoginDTO.getEmployees().getDomains().getDomainId(), ActiveStatus.Activated));
    ...
    public ResponseEntity<Page<ProfileDTO>> search(Principal principal,
                                                   ...
                                                   @RequestParam(value = "stats", defaultValue = "", required = false) String status,
