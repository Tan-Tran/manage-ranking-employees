...
public class ProfilePatternsController {
    ...
    public ModelAndView statusMemberProfile(@PathVariable(value = "idEmployee") Integer userId, ModelAndView modelAndView) {
        employeeService.statusMemberProfile(userId);
        modelAndView.setViewName("redirect:/profile-pattern/updateDomainEmployee");
        return modelAndView;
    }
    ...
    public ModelAndView updateEmployeeRoles(@RequestParam("employeeId") int employeeId,
        ...
        employees.setJobRoles(jobRolesService.findFirstJobRoleByDomainId(domains.getDomainId()));
        employeeService.updateEmployee(employees);
        List<UserAccounts> userAccountsList = userAccountsService.findAllUserAccounts();
        userAccountsService.sentEmailChangeDomain(employees, domainName);
        modelAndView.addObject("userAccountsList", userAccountsList);
   ...
   public ResponseEntity<Page<PatternDTO>> search(
      ...
      @RequestParam(value = "activeStatusPattern", defaultValue = "") String status,
