...
//                }
        ...
        if (userAccountsLoginDTO.getUserRoles().getUserRoleName().equals("ROLE_LEADER")) {
            ...
            if (this.checkConditionProfileToApprove(competencyRankingProfilesToApprove)) {
                if (!cmtApprove.equals("")) {
                    ...
                    httpStatus = HttpStatus.OK;
                    this.sendMailToEmployee(userAccountsLoginDTO, competencyRankingProfilesToApprove, "Approved");
                } else {
        ...
        if (userAccountsLoginDTO.getUserRoles().getUserRoleName().equals("ROLE_LEADER")) {
            if (this.checkConditionProfileToReview(competencyRankingProfilesToReject)) {
                if (!cmtReject.equals("")) {
                    ...
                    httpStatus = HttpStatus.OK;
                    this.sendMailToEmployee(userAccountsLoginDTO, competencyRankingProfilesToReject, "Reviewed Fail");
                } else {
            ...
            if (this.checkConditionProfileToApprove(competencyRankingProfilesToReject)) {
                if (!cmtReject.equals("")) {
                    ...
                    httpStatus = HttpStatus.OK;
                    this.sendMailToEmployee(userAccountsLoginDTO, competencyRankingProfilesToReject, "Rejected");
                } else {
    ...
    public void sendMailToEmployee(UserAccountsLoginDTO userAccountsLoginDTO, CompetencyRankingProfiles competencyRankingProfiles, String contentMail){
        String role = "";
        String newRankContent = "";
        if (userAccountsLoginDTO.getUserRoles().getUserRoleName().equals("ROLE_MANAGER")) {
            role = "Manager";
            String jobRole = competencyRankingProfiles.getCompetencyRankingPatterns().getJobRoles().getJobRoleName();
            String domain = competencyRankingProfiles.getCompetencyRankingPatterns().getJobRoles().getDomains().getDomainName();
            newRankContent = "Your new Rank is " + jobRole + " " + domain + ".<br>";
        }
        if (userAccountsLoginDTO.getUserRoles().getUserRoleName().equals("ROLE_LEADER")) {
            role = "Leader";
        }
        String userName = competencyRankingProfiles.getEmployees().getFullName();
        EmailTemplate emailTemplate = new EmailTemplate();
        String link = "http://localhost:8080/new-profile/detail/" + competencyRankingProfiles.getRankingProfileId();
        String content =  "Dear " + userName + ",<br><br>"
                        + "Your Profile: " + competencyRankingProfiles.getProfileTitle() + " has been " + contentMail + " by " + role + " " +
                                    userAccountsLoginDTO.getEmployees().getFullName() + ".<br>"
                        + newRankContent
                        + "Click here to view your Profile: <b><a href=" + link + ">Profile</a></b><br>"
                        + "Thank you!<br>";
        emailTemplate.setFromAddress("fpt.academy@gmail.com");
        emailTemplate.setToAddress(competencyRankingProfiles.getEmployees().getUserAccounts().getEmail());
        emailTemplate.setSubject("Profile " + contentMail);
