...
public class UserAccountsServiceImpl implements UserAccountsService, UserDetailsService {
    ...
    public void saveAccount(UserAccountsLoginDTO userAccountsLoginDTO) {
        if (!userAccountsRepository.existsByEmail(userAccountsLoginDTO.getEmail())) {
            ...
            userAccountsLoginDTO.getEmployees().setUserAccounts(userAccounts);
            userAccounts.setEmployees(userAccountsLoginDTO.getEmployees());
            userAccounts.getEmployees().setUserAccounts(userAccounts);
    ...
    @Override
    public void sentEmailChangeDomain(Employees employees, String domainName){
        EmailTemplate emailTemplate = new EmailTemplate();
        String content =  "Dear " + employees.getFullName() + ",<br>"
                        + "Your Domain has been change to " + domainName + " by The Manager.<br>"
                        + "Thank you.<br>";
        emailTemplate.setFromAddress("fpt.academy@gmail.com");
        emailTemplate.setToAddress(employees.getUserAccounts().getEmail());
        emailTemplate.setSubject("Job role change!");
