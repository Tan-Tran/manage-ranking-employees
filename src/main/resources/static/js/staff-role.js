...
function searchList(index) {
    $(document).ready(function () {
        ...
        $.ajax({
            ...
            type: "get",
            async: false,
            data: { "title": title.value, "period": period.value, "createdDate": createdDate.value, "stats": stats.value, "roles": roles.value },
        ...
        $.ajax({
            ...
            success: function (wrapperList) {
                ...
                list.forEach(profileDetails => {
                    ...
                    selectHTML = "<select class='dataSources text-center' field='" + profileDetails.dataSources + "' style='width: 100%'>" + sourceList + "</select>\n";
                    // List Levels
                    lvlList = "";
                    ...
                    val = " <tr class='detailList'>\n" +
                        "   <td field='" + profileDetails.competencyRankingPatternDetails + "' class='text-left' value='" + profileDetails.competencyRankingPatternDetails.competencyPatternDetailId + "'>\n" +
                        "   " + detail + "</td>\n" +
                        "   <!--Source info-->\n" +
                        "   <td class='text-center'>\n" +
                        selectHTML +
