...
function showPatternDetail(location) {
    ...
    let componentName = $(location).html().trim();
...
function searchListPattern(index) {
    ...
    $.ajax({
        ...
        type: "get",
        async: false,
        data: {title: title.value, domainName: domainName.value, jobRolesName: jobRolesName.value, period: period.value, activeStatusPattern: activeStatusPattern.value},
        success: function (result) {
            let list = result.content;
            if ((index > 0) && (list.length ===0)){
                let nextPage = index - 1;
                if (nextPage >= 0){
                    searchListPattern(nextPage);
                }
                else {
                    searchListPattern(0);
                }
